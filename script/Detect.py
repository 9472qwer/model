...
from test2 import crawl,crawl_int
...
ctimes = 0
model = None  # 全局变量，用于存储 YoloV5 模型
my_global_arrays = []
...
def ini():
    ...
    model = YoloV5(yolov5_yaml_path='config/number_detection.yaml')
    ...
    while True:
        ...
        if xyxy_list:
            for i in range(len(xyxy_list)):
                ...
                camera_xyz = camera_xyz * 1000  # 换算单位为mm
        ...
        print('相机坐标系 camera_xyz_list：',camera_xyz_list)

        robot_xyz_list = [] #机械爪抓取坐标
        if camera_xyz_list:
            for i in range(len(camera_xyz_list)):
                robot_x = camera_xyz_list[i][1] - 55
                robot_y = camera_xyz_list[i][0] - 410
                robot_z = 150
                robot_xyz = [robot_x, robot_y, robot_z, -180, 0, 0]
                robot_xyz_list.append(robot_xyz)
        else:
            flag = flag + 1
            time.sleep(1)
            if flag > 1:
        ...
        return robot_xyz_list
        # # 进行抓取并移动
        # if robot_xyz_list:
        #     for i in range(len(robot_xyz_list)):
        #         global ctimes
        #         crawl(robot_xyz_list[i],ctimes)
        #         ctimes+=1
        #
        # # 添加fps显示
